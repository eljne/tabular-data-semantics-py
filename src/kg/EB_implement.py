...
db_noun_list_flt = filter_SW(db_noun_list, ' ')
wd_noun_list_flt = filter_SW(wd_noun_list, ' ')
db_noun_phrase_list_flt = filter_SW(db_noun_phrase_list, ',')
wd_noun_phrase_list_flt = filter_SW(wd_noun_phrase_list, ',')
...
db_ent_n = get_entities(db_noun_list_flt)
wd_ent_n = get_entities(wd_noun_list_flt)


def get_entities_np(lst):
    all_entities = []
    limit = 1
    for phrase in lst:
        # look up in DBP KG
        ...
        for ent in entities:
            # print(phrase, 'DBPedia', ent)
            all_entities.append(ent)
    return all_entities


db_ent_np = get_entities_np(db_noun_phrase_list_flt)
wd_ent_np = get_entities_np(wd_noun_phrase_list_flt)
